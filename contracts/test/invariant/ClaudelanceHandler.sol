...
///         Each external function below is a randomized state transition the fuzzer
///         may select. Bounded inputs keep the search space realistic. The handler
///         tracks per-actor cUSD deposited and per-actor cUSD withdrawn so the
///         invariant test can assert conservation of value.
contract ClaudelanceHandler is CommonBase, StdCheats, StdUtils {
    ...
    constructor(ClaudelanceCore _core, MockCUSD _cusd, address _treasury, address _relayer, address[] memory _actors) {
        core = _core;
        cusd = _cusd;
        treasury = _treasury;
        relayer = _relayer;
    ...
    function postBounty(
        ...
        stake = uint96(bound(stake, 0, MAX_STAKE));
        ...
        vm.prank(poster);
        try core.postBounty(0, "g/h", "g/h/i/1", bytes32(0), amount, maxSlots, stake, deadline, ciRequired)
        returns (uint256 id) {
            bountyIds.push(id);
        } catch {
    ...
    function withdrawEarnings(uint256 actorSeed) external countCall("withdrawEarnings") {
        ...
        uint256 owed = core.earnings(who);
        vm.prank(who);
        try core.withdrawEarnings() {
            totalWithdrawnByActors += owed;
        } catch {}
    }

    function withdrawTreasury() external countCall("withdrawTreasury") {
        uint256 owed = core.earnings(treasury);
        if (owed == 0) return;
        vm.prank(treasury);
        try core.withdrawEarnings() {
            totalWithdrawnByActors += owed;
